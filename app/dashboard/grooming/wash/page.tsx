...
<h1 style={{ fontSize: '32px', fontWeight: 800, color: '#000', margin: '0 0 24px' }}>Yıkama Girişi</h1>
        ...
        {searchResults.length > 0 && !selectedPet && (
          <div style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid #E5E5EA', marginBottom: '8px' }}>
            {searchResults.map((pet, idx) => (
              <div key={pet.id} onClick={() => { setSelectedPet(pet); setSearch(pet.name); setSearchResults([]); const s = (pet.species ?? '').toLowerCase()
setField('species', s === 'cat' || s === 'kedi' ? 1 : s === 'dog' || s === 'köpek' || s === 'kopek' ? 0 : 2) }} style={{ padding: '10px 16px', borderBottom: idx < searchResults.length - 1 ? '1px solid #E5E5EA' : 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}>
...
>
  ✓ {loading ? 'Kaydediliyor...' : 'Yıkamayı Kaydet'}
</button>
    </div>
  )
}